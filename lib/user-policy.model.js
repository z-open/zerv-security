...
function UserPolicy(securityData, getPolicyConditionFactory, getResourceTypeFactory) {
    ...
    var protectedResourceList = compileSecurityData(securityData);
    ...
    this.getProtectedResourceByLocator = function(locator) {
        ...
        var protectedResource = _.find(protectedResourceList, function(r) {
    ...
    function compileSecurityData(securityData) {
        var protectedResourceMap = buildProtectedResources(
    ...
    function buildProtectedResources(dictionary, resourceTypes) {
        var resourceMap = {};
        dictionary.forEach(function(resourceData) {
            var resource = _.assign({}, resourceData);
    ...
    function addProtectedResourcePreferences(policies, protectedResources) {
        policies.forEach(function(policy) {
            policy.settings.forEach(function(policySettingConfig) {
                var policySetting = _.assign({}, policySettingConfig);
                policySetting.checkIfEnabled = getPolicyConditionImplementation(policySetting);
                policySetting.policy = policy;

                policySetting.protectedResources.forEach(function(policyResourceConfig) {
                    try {
    ...
    function addPolicyResourceSettingToProtectedResource(policyResourceConfig, protectedResource, policySetting) {
        ...
        var resourceConfig = _.assign({}, policyResourceConfig);
    ...
    */
    ...
    function buildResourceSettingObject(resource, setting, params) {
        var settingObject = _.find(resource.type.settings, { value: setting });
    ...
    */
    function buildProtectedResourceObject(resource) {
        ...
        var implementation = getResourceImplementation(resource.type);
    ...
    function getPolicyConditionImplementation(policySetting) {
        ...
        try {
            var period = policySetting.condition.indexOf('.');
            if (period === -1) {
                throw new Error('No security name defined');
            }
            var factoryName = policySetting.condition.substring(0, period);
            var conditionGroup = getPolicyConditionFactory(factoryName);
            var conditionName = policySetting.condition.substring(period + 1);
            var conditionFn = conditionGroup[conditionName];
            if (!conditionFn) {
