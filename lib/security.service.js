...
var logger = zlog.getLogger('zerv/security');
...
function applyResourcePolicy(user, protectedResourceLocator, contextParams) {
    ...
    return collectServerUserPolicy(user).then(function(userPolicy) {
        ...
        var protectedResource = userPolicy.getProtectedResourceByLocator(protectedResourceLocator);
        //return Promise.reject('PROTECTED_RESOURCE_UNDEFINED');

        // let's calculate the setting to know how this resource should behave        
        var setting = protectedResource.calculateSetting(contextParams);
...
function collectServerUserPolicy(user) {
    return findUserPolicyData(user)
        .then(function(userPolicies) {
            var securityData = formatUserSecurityData(generateUserSecurityDataByEnvironment(user, userPolicies, 'server'));
...
function generateUserSecurityDataByEnvironment(user, userPolicies, env) {
    ...
    var filteredPolicies = filterPolicyContentByEnvironment(userPolicies, env);
    var filteredDictionary = _.filter(systemSecurityData.dictionary, function(resource) {
        return systemSecurityData.resourceTypes.find(resource.type).env.indexOf(env) !== -1;
    });
    var filteredResourceTypes = _.filter(systemSecurityData.resourceTypes, function(type) {
...
function formatUserSecurityData(securityData) {
    var formated = {
        id: securityData.user.id, // a user can only have access to one record
        revision: Date.now(),
        timestamp: {},
    ...
    logger.debug('--------------------------------------------------');
    logger.info(securityData.user.display + ': ' + securityData.env.toUpperCase() + ' Server Security: ' + (securityData.user.permissionRoleCode || 'Role has not been defined'));
...
function filterPolicyContentByEnvironment(userPolicies, environment) {
    ...
    userPolicies.forEach(function(policy) {
        ...
        var filteredPolicySettings = [];
        policy.settings.forEach(function(policySetting) {
            var filteredProtectedResources = _.filter(policySetting.protectedResources,
                function(protectedResource) {
                    var resource = systemSecurityData.dictionary.findProtectedResourceByName(protectedResource.resource);
            ...
            if (filteredProtectedResources.length > 0) {
                var filteredPolicySetting = _.assign({}, policySetting);
        ...
        if (filteredPolicySettings.length > 0) {
            var filteredPolicy = _.assign({}, policy);
...
function collectRolePolicySettingDefinitions(rolePolicySettings, policyDefinition) {
    return _.map(rolePolicySettings, function(setting) {
        var policySettingConfiguration;
        ...
        if (setting.params) {
            var c = _.assign({}, policySettingConfiguration);
